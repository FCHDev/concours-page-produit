...
const Navbar = ({cartCounter}) => {
    ...
    return (
        <div className="w-full h-16 md:flex rounded-xl md:my-3 px-3 md:px-0">
            <ul className="flex h-full text-white md:w-3/4 md:mx-auto md:justify-evenly justify-between items-center">
                {menuItems.map(item => (
                    <li key={v4()} className="md:text-2xl text-base cursor-pointer font-[Alexandria] test">{item}</li>
                ))}
            </ul>
